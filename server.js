...
async function postWithTimeout(url, path, payload, timeout = 300) {
...
async function replicateToFollower(peerUrl, entryIndex) {
  ...
  try {
    const response = await postWithTimeout(peerUrl, "/append-entries", {
      term: currentTerm,
      leaderId: NODE_ID,
      prevLogIndex,
      prevLogTerm,
    ...
    const conflictIndex = Number.isInteger(data.conflictIndex) ? data.conflictIndex : 0;
    const missingEntries = log.slice(conflictIndex);

    const syncResponse = await postWithTimeout(peerUrl, "/sync-log", {
      term: currentTerm,
      leaderId: NODE_ID,
      startIndex: conflictIndex,
      entries: missingEntries,
      leaderCommit: commitIndex,
    });

    return Boolean(syncResponse.data.success);
  } catch (error) {
    return false;
  }
}
...
async function commitClientCommand(req, res) {
  ...
  return res.status(503).json({ ok: false, committed: false, ackCount, needed: majorityCount() });
}
